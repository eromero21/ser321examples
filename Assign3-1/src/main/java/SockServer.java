...
public class SockServer {
  ...
  static JSONArray movies = new JSONArray();
  static JSONArray raters = new JSONArray();
  ...
  static JSONObject addMovie(JSONObject req) {
	  System.out.println("Add movie request: " + req.toString());
	  JSONObject movie = testField(req, "movie");
	  JSONObject rating = testField(req, "rating");
	  JSONObject user = testField(req, "username");
	  
	  if(!movie.getBoolean("ok")) {
		  return movie;
	  } else if(!rating.getBoolean("ok")) {
		  return rating;
	  ...
	  try {
		  res.put("movie", req.getString("movie"));
          res.put("rating", req.getInt("rating"));
          res.put("raters", raters.put(req.getString("username")));
	  } catch(JSONException e) { 
		  res.put("ok", false);
		  res.put("message", "Field movie/username must be String, rating must be int.");
          return res;
	  }
      ...
      retObject.put("type", "rating");
      retObject.put("ok", true);
      retObject.put("movies", movies);
      retObject.put("message", "Working as intended.");
